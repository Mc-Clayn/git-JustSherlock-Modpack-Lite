...
begin
 ...
 if not FilesExists([WizardDirValue() + '\WorldOfTanks.exe', WizardDirValue() + '\version.xml', WizardDirValue() + '\app_type.xml']) then      // '\Tanki.exe'
 begin
 ...
 begin
  ...
  begin
    ...
    // check for Lesta Client gameVersion
    // its work
    // if in Dir choose Game Lesta we get PatchLesta, if WG - PatchWG
    ItemCaption := WOTList.Text;
    IsLesta := @CHECK_IsLesta;
    if (pos('Lesta', ItemCaption) <> 0) then
      IsLesta := True
    else
      IsLesta := False;
    //IsLesta := True;//@CHECK_IsLesta;
    if not IsLesta then
      //GamePatch := '{#PatchWG}'
      ggPatch('{#PatchWG}')
    else
      //GamePatch := '{#PatchLesta}';
      ggPatch('{#PatchLesta}');
